...
{
    //? this controller related to admin panel for add network
    // listing Network
    public function index(Request $request)
    {
        if ($request->ajax()) {
            // Get all Network
            $data = Network::orderBy('created_at', 'desc')->get();
            // Get yajra dataTable
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $editUrl = 'networks-list/' . $row['network_id'] . '/edit';
                    $deleteUrl = 'networks-list/' . $row['network_id'];
        ...
        return view('Admin.networks.index_networks');
    }
    // Get all_sponsored_networks
    public function all_sponsored_networks(AdminController $ac)
    {
        // Get all_sponsored_networks
        $data = $ac->all_sponsored_networks();
        return view('Admin.networks.all_sponsored', [
            'data' => $data
        ]);
    }
    // Get all_top_networks
    public function all_top_networks(AdminController $ac)
    {
        // Get all_top_networks
        $data = $ac->all_top_networks();
        return view('Admin.networks.all_top_networks', [
            'data' => $data
        ]);
    }
    // Get featured_networks
    public function featured_networks(AdminController $ac)
    {
        // Get featured_networks
        $data = $ac->featured_networks();
        return view('Admin.networks.featured_networks', [
            'data' => $data
        ]);
    }
    ...
    public function create(NetworkWebController $nwc)
    {
        ...
        return view('Admin.networks.create_network', $data);
    }
    ...
    public function update(Request $request, string $network_id)
    {
        ...
        return redirect('admin/networks-list')->with('message', 'Your data successfully updated');
    }
    ...
    public function destroy(string $network_id)
    {
        ...
        $network->delete();
        return redirect('admin/networks-list')->with('message', 'Your data has been successfully deleted');
    }
    ...
    public function bulkAction(Request $request)
    {
        ...
        if ($action === 'delete') {
            if (!empty($selectedItems)) {
                Network::whereIn('network_id', $selectedItems)->delete();
